...
$(document).on("submit","#adminLoginFrm", function(){
 ...
 // Check if the user has a color scheme preference stored
if (localStorage.getItem('color-scheme') === 'dark') {
  // Apply dark color scheme
  document.documentElement.setAttribute('data-theme', 'dark');
}

// Listen for changes to the color scheme selector and update the preference
document.querySelector('#color-scheme-selector').addEventListener('change', function(event) {
 ...
 $(document).on("submit","#updateExamineeFrm" , function(){
   $.post("updateExamineeExe.php", $(this).serialize() , function(data){
      if(data.res == "success")
      {
         Swal.fire(
             'Success',
